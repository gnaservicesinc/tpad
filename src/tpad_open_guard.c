...
if(iswpFilePathSize >  _FP_BUFFER_SIZE) return(-1);
	...
	strncpy(path_temp_file,(gchar*) g_strconcat(currentfile,"~",NULL),sizeof(path_temp_file));		
...
void tpad_open_guard_attack(gchar* fp){
	if (!gerror_openguard_popup(currentfile, fp)){
			new_thread_tpad(g_strdup(currentfile));
			exit(1);
	}
}
...
if(ihaveTemp == 0 && currentfile && path_temp_file) {
		FILE * origFile=NULL;
		long lSize;
		char * buffer;
		size_t result;
		origFile = fopen (currentfile,"rb");

		if (origFile==NULL) exit(1);

		fseek (origFile , 0 , SEEK_END);
		lSize = ftell (origFile);
		rewind (origFile);
